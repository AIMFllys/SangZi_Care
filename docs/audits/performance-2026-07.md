...
- Vitest：81 个测试文件通过，832 项通过；1 个付费 MiMo 实时测试默认跳过。
...
- 最终连续直达路由验收发现：整页导航可能取消上一文档仍在途的 `/users/me`，旧实现会把该临时异常误判为凭证失效并删除会话。现已收紧为仅在明确 401 时清除凭证；导航取消、直接网络错误和非 401 上游失败会保留可恢复会话。对应回归测试先失败后修复通过，并保留“401 必须退出”的反向断言。
