...
> 技术栈: Next.js ≥16.2 + React 19 + Zustand 5 + Supabase（目标全栈 Route Handlers）  
> 架构目标: **EdgeOne 全栈 Next.js** · Android **在线壳** · CSS Modules 为主 + Tailwind v4 工具层  
> 遗留: `backend/` Python FastAPI **只读对照**（过渡期仍可占 8000 端口）  
> 开发端口: Next **7742**  
...
├── app/                    # Next App Router 页面 + app/api（目标）
...
App Router 文件系统路由。业务数据一律经 FastAPI，不直连 Supabase CRUD（Realtime 客户端另见 `lib/`）。
...
- 过渡期动态路由若仍带 placeholder，随全栈 SSR/CSR 自然缓解
...
| `useVoiceRecognition.ts` | Web → Native → 豆包 ASR | `/health/input` |
| `useTextToSpeech.ts` | Web → Native → 豆包 TTS | ChatDetail |
...
| `jsbridge.ts` | 期望 `window.AndroidBridge` | 与原生 `SangZiBridge` **契约不一致** |
| `voiceCapabilities.ts` | 检测 Web/Native/Doubao 能力 | voiceStore |
...
> **目标**：业务 API 迁入 Next `app/api/**`。本目录仅对照，见 [backend/README.md](../../backend/README.md)。

FastAPI + Supabase **PostgREST**（`SyncPostgrestClient` + 旧 service role；迁移期改用 secret key）。

```
backend/
├── main.py                 # CORS、全局异常、挂载 /api/v1、GET /health
├── requirements.txt
├── database-init.sql       # 历史 DDL（相对运行时代码偏旧）
├── migration-*.sql         # 增量迁移
│
├── core/
│   ├── config.py           # 环境变量键（无密钥写入文档）
│   ├── security.py         # JWT 签发/校验
│   └── middleware.py       # require_auth / optional_auth（Depends，非 HTTP 中间件）
│
├── models/                 # Pydantic：user、health、medicine、message、
│                           # broadcast、emergency、ai
├── api/v1/                 # 见下表
├── services/
│   ├── supabase_client.py
│   ├── doubao_service.py   # 豆包对话 / 意图 / 摘要
│   ├── voice_service.py    # TTS/ASR 接口稳定，实现多为占位
│   ├── email_service.py    # 阿里云 SMTP 验证码
│   └── health_broadcast.py
└── tests/                  # 按模块的 API / 服务测试
```
...
| 前缀 | 文件 | 核心能力 |
|------|------|----------|
| `/api/v1/auth` | `auth.py` | captcha、send-code、verify、refresh |
| `/api/v1/users` | `users.py` | me、PATCH me、PATCH role |
| `/api/v1/family` | `family.py` | generate-code、bind、binds CRUD |
| `/api/v1/messages` | `messages.py` | 会话、send、send-voice、read、unread |
| `/api/v1/medicine` | `medicine.py` | plans、today、records、notify-family |
| `/api/v1/health` | `health.py` | records CRUD、latest、trend |
| `/api/v1/ai` | `ai_chat.py` | chat、intent、summary、WS voice-session |
| `/api/v1/voice` | `ai_voice.py` | tts、transcribe、WS stream-asr |
| `/api/v1/emergency` | `emergency.py` | trigger、cancel、notify、history |
| `/api/v1/radio` | `radio.py` | recommend、categories、play-record、generate |

另：`GET /health`（根路径探活，无鉴权）。HTTP + WebSocket 合计约 **45** 条路由。

### 配置键（仅列名 · 目标命名）

`NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`、`SUPABASE_SECRET_KEY`（替代旧 anon/service_role）、`JWT_SECRET`、`VOLCANO_*`、`SMTP_*`、过渡期 `NEXT_PUBLIC_API_BASE_URL`。详见 [ops/env-config.md](../ops/env-config.md)。

### 安全要点（结构视角）

- 多数业务路由：`Depends(require_auth)` Bearer JWT
- WebSocket 语音相关：**无 JWT**
...
**模式 B**：WebView 加载可配置的 **https** 基址（EdgeOne 或本地调试 URL），不内嵌 `out/`。

- 原生桥：`SangZiBridge`；前端 `jsbridge.ts` 仍期望 `AndroidBridge` → 见 [tech-debt](../issues/tech-debt.md)
...
flowchart TB
  ...
  FastAPI[FastAPI_legacy_readonly]
  EdgeNext[Next_Route_Handlers]
  SB[(Supabase)]
  Doubao[Doubao_LLM]
  VoiceStub[voice_service_placeholder]
  UI --> Shell
  Shell --> Stores
  Stores --> Api
  Api -->|"过渡期"| FastAPI
...
| 区域 | 框架 / 位置 |
|------|-------------|
| 前端 | Vitest + Testing Library：`hooks/__tests__`、`lib/__tests__`、`stores/__tests__`、`app/**/__tests__`、`components/**/__tests__` |
| 后端 | `backend/tests/`：auth/users/family/messages/medicine/health/emergency/radio/ai/voice/models/security 等 |

`package.json` 未内置 `test` script；本地通常直接 `npx vitest` / `pytest`。后端 `test_auth` 等与现网邮箱+验证码流程可能不同步。
...
|----|------|
| `users` | 用户资料与适老化偏好 |
| `elder_family_binds` | 绑定码、关系、权限、状态 |
| `health_records` | 体征 JSON `values`、异常标记 |
| `medication_plans` / `medication_records` | 计划与服药记录 |
| `elder_care_messages` | 捂话 |
| `emergency_calls` | 紧急呼叫记录 |
| `health_broadcasts` / `broadcast_play_history` | 广播与播放史 |
| `ai_conversations` | AI 对话持久化 |

仓库内 `database-init.sql` 相对上述列名/表名有漂移；另见根目录 `DATABASE_MIGRATION_OC_PREFIX.md`（`oc_` 前缀迁移清单，代码侧未必已切）。
