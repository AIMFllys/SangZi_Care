...
历史 `backend/` 下的 `database-init.sql` / `migration-*.sql` 已删除；新的 schema 变更从 `supabase/migrations/20260713230000_auth_challenges.sql` 起随仓库审计。2026-07-14 的家庭权限、绑定并发、用药发生项与看板 RPC 迁移已通过 MCP 应用到当前目标 Supabase；新环境仍必须按文件顺序应用全部 migration。映射说明见 [docs/ops/oc-table-prefix.md](../ops/oc-table-prefix.md)。
