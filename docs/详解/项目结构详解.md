...
| `/api/v1/ai` | `app/api/v1/ai/**` | chat、intent、summary、WS voice-session |
| `/api/v1/voice` | `app/api/v1/voice/**` | tts、transcribe、WS stream-asr |
...
另：Next 侧提供 `GET /api/ping`（同源探活，无鉴权）。WebSocket 类端点见 `app/api/v1/voice` 与 `app/api/v1/ai`。

### 配置键（仅列名 · 目标命名）

`NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`、`SUPABASE_SECRET_KEY`（替代旧 anon/service_role）、`JWT_SECRET`、`VOLCANO_*`、`SMTP_*`。详见 [ops/env-config.md](../ops/env-config.md)。

### 安全要点（结构视角）

- 多数业务路由：Bearer JWT（`lib/server/auth.ts`）
...
| 服务端 | `lib/server/__tests__`（auth 等）|

`package.json` 未内置 `test` script；本地通常直接 `npx vitest`。历史 Python `backend/tests/` 已随 `backend/` 一并删除。
...
历史 `database-init.sql` / `migration-*.sql` 已随 `backend/` 一并删除；线上表结构以 Supabase 控制台为准。另见根目录 `DATABASE_MIGRATION_OC_PREFIX.md`（`oc_` 前缀迁移清单，代码侧未必已切）。
