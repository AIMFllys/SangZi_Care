...
App Router 文件系统路由。业务数据经 Next.js Route Handlers（`app/api`），不直连 Supabase CRUD。
...
|------|-----|-------|--------|------|
| `/` | ✓ | 语音优先首页 | 看板首页 | 角色分流首页 |
| `/login` | 隐藏 | 共用 | 共用 | 分步邮箱 OTP |
| `/questionnaire` | — | ✓ | ✓ | 健康早筛问卷 |
| `/onboarding` | 隐藏 | 共用 | 共用 | 角色选择 |
| `/health` | ✓ | 本人健康 | 已绑定长辈健康 | 健康看板 |
| `/health/input` | — | 本人录入 | 获授权后代录 | 健康录入 |
| `/medicine` | Elder 中心 | 本人提醒与计划 | 长辈计划管理 | 今日用药 |
| `/medicine/history` | — | 本人 | 当前选中长辈 | 用药历史 |
| `/messages` | ✓ | 「亲属」 | 「消息」 | 联系人 |
| `/messages/[id]` | — | ✓ | ✓ | 聊天详情 |
| `/radio` | — | 直达 | 直达 | 健康广播 |
...
│   └── FamilyHomeView.tsx  # 照护长辈 / 权限分域 / 聚合统计图
├── family/
│   ├── CareRecipientTabs.tsx
│   ├── AdherenceMiniChart.tsx
│   └── HealthSparkline.tsx
├── ui/
│   ├── Button.tsx · Input.tsx · LoadingSpinner.tsx
│   ├── DataStateWrapper.tsx · Skeleton.tsx
│   └── index.ts
└── messages/
    ├── ContactPreferenceDialog.tsx # 私有备注/置顶编辑与移动端操作面板
    ├── MessageList.tsx
...
| `useFamilyBinds.ts` | 拉取绑定列表、错误与重试 | 照护页面、messages |
| `useCareRecipient.ts` | 跨页面解析当前照护长辈与权限 | 首页、健康、用药 |
| `useCareDashboard.ts` | 单请求加载聚合看板并隔离迟到响应 | FamilyHomeView |
| `useAIChat.ts` | MiMo 陪伴对话 / 意图 / 摘要 API | `/voice`（对话为主） |
...
| 模块 | 职责 | 状态 |
|------|------|------|
| `api.ts` | `fetchApi`：Bearer、401 刷新、同账号并发 GET 合并 | 核心，全 Store 使用 |
| `constants.ts` | `ROUTES`、健康阈值 | 使用中 |
| `supabase.ts` | 浏览器 Supabase 客户端 | 认证与查询 |
...
| `/api/v1/messages` | `app/api/v1/messages/**` | 会话、联系人私有偏好、send、send-voice、read-all、unread |
...
另：Next 侧提供 `GET /api/ping`（同源探活，无鉴权、`no-store`），并返回构建时注入的 40 位 Git `revision`，用于核对 EdgeOne 部署与 Release APK 源码。当前语音与 AI 都使用普通同源 HTTP Route Handler，没有 WebSocket 端点。
...
- 服务角色绕过 RLS；跨用户健康查看/代录、用药管理、消息互发均校验 active 家庭绑定与对应权限
...
flowchart TB
  ...
  SB[(Supabase)]
  Doubao[Legacy_intent_summary]
  MiMo[MiMo_v2_5_Pro_ASR_TTS]
  UI --> Shell
  Shell --> Stores
  Stores --> Api
  Api -->|"同源"| EdgeNext
...
页面切换：`app/template.tsx` 为内容区淡入；TabBar 不参与。晚加载走 `components/ui/Skeleton`。
...
| `oc_contact_preferences` | 以 owner/peer 复合键隔离的联系人备注与置顶，仅 service role 访问 |
| `oc_health_records` | 体征 JSON `values`、异常标记 |
| `oc_medication_plans` / `oc_medication_records` | 计划与持久化应服发生项/确认记录 |
| `oc_elder_care_messages` | 家庭消息；`category` 区分 chat / murmur / system |
| `oc_emergency_calls` | 紧急呼叫记录 |
| `oc_health_broadcasts` / `oc_broadcast_play_history` | 广播与播放史 |
| `oc_ai_conversations` | AI 对话持久化 |
| `oc_ai_murmurs` | AI 整理的私密碎碎念与分享审计（强制 RLS） |

历史 `backend/` 下的 `database-init.sql` / `migration-*.sql` 已删除；新的 schema 变更从 `supabase/migrations/20260713230000_auth_challenges.sql` 起随仓库审计。2026-07-14 的家庭权限、绑定并发、用药发生项、看板 RPC、消息分类与碎碎念原子分享迁移均已通过 MCP 应用到当前目标 Supabase；新环境仍必须按文件顺序应用全部 migration。映射说明见 [docs/ops/oc-table-prefix.md](../ops/oc-table-prefix.md)。
