...
import { ArrowLeft, Mic, Search, Drama, HeartPulse, Newspaper, BookOpen, Radio as RadioIcon, Music, Play, Pause } from 'lucide-react';
import styles from './page.module.css';

const CATEGORIES = [
  { name: '京剧名段', icon: <Drama size={24} />, color: styles.catOrange },
  { name: '养生常识', icon: <HeartPulse size={24} />, color: styles.catGreen },
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <button className={styles.backBtn} onClick={() => router.back()} aria-label="返回首页"><ArrowLeft size={24} /></button>
      ...
      <div className={`glass-card ${styles.searchBar}`}>
        ...
        <input
          ...
          readOnly
        />
        <button className={styles.searchBtn} aria-label="搜索"><Search size={20} /></button>
      </div>
        ...
        <div className={styles.recommendList}>
          {broadcasts.map((item, i) => (
            <div key={item.id} className={`glass-card ${styles.recommendItem} interactive`}>
              <div className={styles.recommendIcon}><Music size={24} color="var(--accent)" /></div>
              ...
              <button className={styles.playBtn} aria-label="播放"><Play size={20} fill="currentColor" /></button>
            </div>
          ))}
        </div>
      </DataStateWrapper>
      ...
      {currentBroadcast && isPlaying && (
        <div className={`glass-card ${styles.player}`}>
          <span><Music size={20} color="var(--accent)" /></span>
          <span className={styles.playerTitle}>{currentBroadcast.title}</span>
          <button className={styles.playerBtn} aria-label={isPlaying ? '暂停' : '播放'}>{isPlaying ? <Pause size={24} fill="currentColor" /> : <Play size={24} fill="currentColor" />}</button>
        </div>
      )}
    </div>
  );
